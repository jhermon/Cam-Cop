...
def click1():
    ...
    if filename == "":
       ...
       resize_image = image_Pil.resize((800, 600), Image.ANTIALIAS)
...
def detector():
    ...
    if len(detect.plate) != 6:
        ...
        detector.label_plate.place(x=1120,y=200)
        detector.label_plate.config(font=('Helvetica',17,'bold'))
    else:
        
        detector.label_plate = Label(root,text=detect.plate)
        detector.label_plate.place(x=1150,y=180)
        detector.label_plate.config(font=('Helvetica',44))
        #this function queries the database
        
        get_details(detect.plate)
        if len(get_details.lst) == 0:
            detector.label_header = Label(root,text='Driver Information not found')
            detector.label_header.place(x=1150,y=270)
            detector.label_header.config(font=('Helvetica',15,'bold'))
        elif type(get_details.lst[0]) == str:
            #adding result of database to the window
            #header of driver details
            detector.label_header = Label(root,text='Driver Information')
            detector.label_header.place(x=1150,y=270)
            detector.label_header.config(font=('Helvetica',25,'bold'))
            #print firstname
            detector.label_firstname = Label(root, text= 'Firstname: ' + get_details.lst[0])
            detector.label_firstname.place(x=1150,y=320)
            detector.label_firstname.config(font=('Helvetica',15))
            #print lastname
            detector.label_lastname = Label(root, text= 'Lastname: ' + get_details.lst[1])
            detector.label_lastname.place(x=1150,y=350)
            detector.label_lastname.config(font=('Helvetica',15))
            #print gender
            detector.label_gender = Label(root, text= 'Gender: ' + get_details.lst[2])
            detector.label_gender.place(x=1150,y=380)
            detector.label_gender.config(font=('Helvetica',15))
            #print TRN
            detector.label_TRN = Label(root, text= 'TRN: ' + get_details.lst[3])
            detector.label_TRN.place(x=1150,y=420)
            detector.label_TRN.config(font=('Helvetica',15))
            #print address
            detector.label_address = Label(root, text= 'Address:')
            detector.label_address2 = Label(root, text= get_details.lst[4])
            detector.label_address.place(x=1150,y=450)
            detector.label_address.config(font=('Helvetica',15))
            detector.label_address2.place(x=1150,y=480)
            detector.label_address2.config(font=('Helvetica',15))
            #print DOB
            dob = get_details.lst[5].strftime('%m/%d/%Y')
            detector.label_DOB = Label(root, text= 'DOB: ' + dob)
            detector.label_DOB.place(x=1150,y=510)
            detector.label_DOB.config(font=('Helvetica',15))
            #print car brand
            detector.label_car_brand = Label(root, text= 'Car brand: ' + get_details.lst[6])
            detector.label_car_brand.place(x=1150,y=540)
            detector.label_car_brand.config(font=('Helvetica',15))
            #print car color
            detector.label_car_color = Label(root, text= 'Car color: ' + get_details.lst[7])
            detector.label_car_color.place(x=1150,y=570)
            detector.label_car_color.config(font=('Helvetica',15))
            #year made
            detector.label_year_made = Label(root, text= 'Year made: ' + get_details.lst[8])
            detector.label_year_made.place(x=1150,y=600)
            detector.label_year_made.config(font=('Helvetica',15))
            #year purchased
            detector.label_year_purchased = Label(root, text= 'Year purchased: ' + get_details.lst[9])
            detector.label_year_purchased.place(x=1150,y=630)
            detector.label_year_purchased.config(font=('Helvetica',15))
        else:
            #get_details(detect.plate)
            detector.label_match = Label(root, text='Possible match found: ' + get_details.lst[0][1] + '\n'+'Percentage match: ' + str(get_details.lst[0][0]) + '%')
            detector.label_match.place(x=1150,y=260)
            detector.label_match.config(font=('Helvetica',15))
...
def reset():
    ...
    elif len(get_details.lst) == 0 and detector.label_img !='' and  len(detect.plate) == 6:
        detector.label_img.destroy()
        detector.label_plate.destroy()
        detector.label_header.destroy()
    elif type(get_details.lst[0][0]) == int and detector.label_img !='' and len(detect.plate) == 6:
        detector.label_img.destroy()
        detector.label_plate.destroy()
        detector.label_match.destroy()    
    elif type(get_details.lst[0]) == str and detector.label_img !='' and len(detect.plate) == 6:
        detector.label_img.destroy()
        detector.label_plate.destroy()
        detector.label_header.destroy()
